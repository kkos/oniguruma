...
POOL_CAST='s/(int *)(long *)&((struct \+unicode_prop_name_pool_t *\* *) *0/(short int)(long)\&((struct unicode_prop_name_pool_t *)0/g'
...
sed -e 's/^#line.*$//g' ${TMP} | sed -e "${POOL_CAST}" > ${NAME}.c
gperf ${GPERF_OPT} -N unicode_lookup_property_name --output-file ${TMP} ${NAME}_posix.gperf
sed -e 's/^#line.*$//g' ${TMP} | sed -e "${POOL_CAST}" > ${NAME}_posix.c
